...
class PagedKVManager(ParamsBase):
    ...
    single_page_affine: cutlass.Constexpr[bool]
    ...
    def create(
        ...
        single_page_affine: cutlass.Constexpr[bool] = False,
    ):
        ...
        tPrSinglePage = cute.make_rmem_tensor((1,), Int32)
    ...
    def load_page_table(self, n_block: Int32):
        if const_expr(self.single_page_affine):
            tile_start = n_block * self.n_block_size
            tile_page_idx, tile_page_offset = divmod(
                tile_start + self.leftpad_k, self.page_size_divmod
            )
            full_single_page = (
                n_block >= 0
                and tile_start + self.n_block_size <= self.seqlen_k
                and tile_page_offset + self.n_block_size
                <= self.page_size_divmod.divisor
            )
            self.tPrSinglePage[0] = 0
            if full_single_page:
                # There are two load warps on the audited SM110 route.  Each
                # warp broadcasts one page-table read without a new barrier.
                page = Int32(0)
                if self.thread_idx % cute.arch.WARP_SIZE == 0:
                    page = self.mPageTable[tile_page_idx]
                page = cute.arch.shuffle_sync(page, 0)
                self.tPrSinglePage[0] = 1
                self.tPrPage[0] = page
                self.tPrPageOffset[0] = tile_page_offset
            else:
                for i in cutlass.range(self.page_entry_per_thread, unroll=1):
                    ...
                    row_page_idx, row_page_offset = divmod(
                    ...
                    page = self.mPageTable[row_page_idx] if is_valid else 0
                    self.tPrPage[i] = page
                    self.tPrPageOffset[i] = row_page_offset
        else:
            # Keep the default-off generated path structurally identical to
            # upstream beta15; the candidate-only fallback lives above.
            for i in cutlass.range(self.page_entry_per_thread, unroll=1):
                row = (
                    i * self.num_threads
                    + (self.thread_idx % self.gmem_threads_per_row)
    ...
    @cute.jit
    def _load_KV_general(
        self,
        tXsX: cute.Tensor,
        tXcX: cute.Tensor,
        tXc0X: cute.Tensor,
        seqlenk_row_limit: Int32,
        ...
        for m in cutlass.range_constexpr(cute.size(tXsX, mode=[1])):
            ...
            mX_paged_cur = cute.make_tensor(
                x_gmem_ptr, cute.make_layout((head_dim,))
            )
            mX_paged_cur_copy = cute.tiled_divide(
                mX_paged_cur, (self.async_copy_elems,)
            )
            self._copy_row_async(
                tXsX, tXcX, mX_paged_cur_copy, m, should_load
            )

    @cute.jit
    def _load_KV_single_page_affine(
        self,
        tXsX: cute.Tensor,
        tXc0X: cute.Tensor,
        seqlenk_row_limit: Int32,
        ...
        # A full tile inside one physical page is a regular 2-D NHD slice.
        # Partitioning that affine tensor removes the per-row 64-bit pointer
        # shuffle while preserving the original cp.async issue order below.
        mX = self.mK_paged if const_expr(K_or_V == "K") else self.mV_paged
        transposed = const_expr(K_or_V == "V" and self.v_gmem_transposed)
        page = self.tPrPage[0]
        page_offset = self.tPrPageOffset[0]
        if const_expr(transposed):
            x0 = utils.elem_pointer(mX, (0, page_offset, page))
            row_stride, k_stride = mX.stride[1], mX.stride[0]
        else:
            x0 = utils.elem_pointer(mX, (page_offset, 0, page))
            row_stride, k_stride = mX.stride[0], mX.stride[1]
        x0 = cute.make_ptr(
            mX.element_type,
            x0.toint(),
            cute.AddressSpace.gmem,
        ...
        gX = cute.make_tensor(
            x0,
            cute.make_layout(
                (self.n_block_size, head_dim),
                stride=(row_stride, k_stride),
            ),
        )
        tXgX = self.gmem_thr_copy_KV.partition_S(gX)
        for m in cutlass.range_constexpr(cute.size(tXsX, mode=[1])):
            row_valid = tXc0X[0, m, 0][0] < seqlenk_row_limit
            should_load = cute.make_fragment_like(
                tXsX[(0, None), m, 0], cute.Boolean
            ...
            should_load.fill(row_valid)
            for k in cutlass.range_constexpr(cute.size(tXsX, mode=[2])):
                tXgX_k = tXgX[None, m, k]
                tXsX_k = tXsX[None, m, k]
                tXgX_k = cute.make_tensor(tXgX_k.iterator, tXsX_k.layout)
                cute.copy(
                    self.gmem_tiled_copy_KV,
                    tXgX_k,
                    tXsX_k,
    ...
    def load_KV(self, n_block: Int32, sX: cute.Tensor, K_or_V: str):
        ...
        if const_expr(not self.single_page_affine):
            tPrXPtr = self.compute_X_ptr(K_or_V)
        ...
        if const_expr(self.single_page_affine):
            if self.tPrSinglePage[0] != 0:
                self._load_KV_single_page_affine(
                    tXsX,
                    tXc0X,
                    seqlenk_row_limit,
                    head_dim,
