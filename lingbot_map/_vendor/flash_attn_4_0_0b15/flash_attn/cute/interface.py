...
def _flash_attn_fwd(
    ...
    # The optional affine loader is compile-keyed and fail-closed.
    candidate023_public = os.environ.get("LINGBOT_THOR_PAGED_KV_AFFINE", "")
    if candidate023_public not in ("", "0", "1"):
        raise RuntimeError(
            "LINGBOT_THOR_PAGED_KV_AFFINE must be unset, '0', or '1'; got "
            f"{candidate023_public!r}"
        )
    candidate023_route = "pagedkv_single_page_affine" if candidate023_public == "1" else ""
    candidate023_single_page_affine = False
    if candidate023_route == "pagedkv_single_page_affine":
        contract_ok = (
            arch == 110
            and os.environ.get("LINGBOT_THOR_FA4_QUERY_STAGING") == "1"
            and q.dtype == torch.bfloat16
            and batch_size == 1
            and total_q in (783, 978, 1005, 1042)
            and page_size == total_q - 6
            and max_seqlen_q == total_q
            and num_head == 16
            and num_head_kv == 16
            and head_dim == 64
            and head_dim_v == 64
            ...
            and page_size in (777, 972, 999, 1036)
            and k.stride() == v.stride()
            and k.stride(-1) == 1
            and k.stride(-2) == head_dim
            and k.stride(-3) == num_head_kv * head_dim
            and k.stride(0)
            in (
                page_size * num_head_kv * head_dim,
                2 * page_size * num_head_kv * head_dim,
            )
            and tile_m == 128
            and tile_n == 128
            and q_stage == 2
        ...
        if not contract_ok:
            raise RuntimeError(
                "Candidate023 pagedkv_single_page_affine contract mismatch; "
                "refusing to fall back silently"
            )
        candidate023_single_page_affine = True
    ...
    compile_key = (
        ...
        candidate023_single_page_affine,
        ...
        if arch // 10 == 8:
            ...
            if qv is not None:
                ...
                fa_fwd = flash_fwd_obj_cls(
                    ...
                    single_page_affine=candidate023_single_page_affine,
                )
        elif arch // 10 == 12:
